...
function ProjectCard({ project, priority = false }: { project: ProjectItem; priority?: boolean }) {
  const fakeUrl = "asylyar.ru";
  const tags = project.categories;
  const stats = project.stats?.slice(0, 2) ?? [];

  return (
    <TransitionLink href="#contact" label={project.name} subtitle={project.sector} className="block group/card">
      <div className="rounded-[24px] overflow-hidden bg-white shadow-[0_2px_20px_rgba(0,0,0,0.06)] md:group-hover/card:shadow-[0_20px_60px_rgba(0,0,0,0.12)] md:group-hover/card:-translate-y-1 transition-all duration-500">
        <div className="p-2.5 flex flex-col gap-2.5">
          {/* macOS Header */}
          <div className="h-10 rounded-xl bg-[#f5f5f5] flex items-center px-4">
            <div className="flex gap-2 w-1/4">
              <div className="w-3 h-3 rounded-full bg-[#e0e0e0] md:group-hover/card:bg-[#ff5f57] transition-colors duration-300" />
              <div className="w-3 h-3 rounded-full bg-[#e0e0e0] md:group-hover/card:bg-[#febc2e] transition-colors duration-300 delay-75" />
              <div className="w-3 h-3 rounded-full bg-[#e0e0e0] md:group-hover/card:bg-[#28c840] transition-colors duration-300 delay-100" />
            </div>
            <div className="flex-1 flex justify-center">
              <div className="bg-black/5 md:group-hover/card:bg-black/[0.08] rounded-md px-3 py-1 w-full max-w-[240px] flex items-center gap-1.5 transition-colors duration-300">
          ...
          <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-[#0a0a0a]">
            <Image
              ...
              fill
              priority={priority}
              className="object-cover transition-all duration-700 ease-out md:group-hover/card:scale-110 md:group-hover/card:brightness-[0.35] md:group-hover/card:blur-[2px]"
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent opacity-0 md:group-hover/card:opacity-100 transition-opacity duration-500 z-10 pointer-events-none" />
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 md:p-8 z-20 pointer-events-none">
              <div className="flex justify-between items-end w-full">
                <div className="space-y-3">
                  {/* Title & sector */}
                  <div className="opacity-0 translate-y-4 md:group-hover/card:opacity-100 md:group-hover/card:translate-y-0 transition-all duration-500 ease-out">
                  ...
                  {/* Stats */}
                  {stats.length > 0 && (
                    <div className="flex gap-6 opacity-0 translate-y-4 md:group-hover/card:opacity-100 md:group-hover/card:translate-y-0 transition-all duration-500 ease-out delay-75">
                      {stats.map((s) => (
                  ...
                  {/* Tags */}
                  <div className="flex flex-wrap gap-1.5 opacity-0 translate-y-4 md:group-hover/card:opacity-100 md:group-hover/card:translate-y-0 transition-all duration-500 ease-out delay-150">
          ...
          {/* На мобилке: контент под картинкой — как на десктопе по ховеру, но статично */}
          <div className="md:hidden border-t border-black/5 pt-4 pb-3 px-3 -mx-0.5 rounded-b-2xl bg-white">
            ...
            {project.excerpt && (
              <p className="text-sm text-[#747474] leading-relaxed line-clamp-3 mb-3">{project.excerpt}</p>
            )}
            {stats.length > 0 && (
              <div className="flex gap-4 mb-3">
                {stats.map((s) => (
                  <div key={s.label} className="flex flex-col gap-0.5">
                    ...
                    <span className="text-[9px] uppercase tracking-wider text-black/40 font-medium">{s.label}</span>
                  </div>
                ))}
              </div>
            )}
            {tags.length > 0 && (
              <div className="flex flex-wrap gap-1.5">
                {tags.slice(0, 3).map((tag, i) => (
                  <span
                    key={i}
                    className="px-2.5 py-1 rounded-full border border-black/10 bg-black/[0.04] text-black/60 text-[10px] font-medium tracking-wide"
                  >
                    {tag}
                  </span>
                ))}
